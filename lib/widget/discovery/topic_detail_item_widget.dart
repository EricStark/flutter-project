...
import 'package:first_flutter/utils/date_util.dart';
import 'package:first_flutter/utils/navigator_util.dart';
import 'package:first_flutter/utils/share_util.dart';
import 'package:first_flutter/widget/expand_more_text_widget.dart';
import 'package:flutter/material.dart';

class TopicDetailItemWidget extends StatelessWidget {
  final TopicDetailItemData model;

  const TopicDetailItemWidget({Key? key, required this.model})
      : super(key: key);
                ...
                Row(
                  children: <Widget>[
                    Text(
                      ...
                      style: TextStyle(
                          color: ConstColor.hitTextColor, fontSize: 12),
                    ),
                    Expanded(
                      child: Text(
  ...
  Widget _videoState() {
    return Row(
      ...
      children: <Widget>[
        ...
        Row(
          children: <Widget>[
            Icon(Icons.favorite_border,
                size: 20, color: ConstColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(
                  '${model.data!.content!.data!.consumption!.collectionCount}',
                  style:
                      TextStyle(fontSize: 12, color: ConstColor.hitTextColor)),
            )
          ],
        ),
        // 评论
        Row(
          children: <Widget>[
            Icon(Icons.message, size: 20, color: ConstColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(
        ...
        Row(
          children: <Widget>[
            Icon(Icons.star_border, size: 20, color: ConstColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(ConstString.collect_text,
                  style:
                      TextStyle(fontSize: 12, color: ConstColor.hitTextColor)),
            ),
          ],
        ...
        // 分享
        IconButton(
          icon: Icon(Icons.share, color: ConstColor.hitTextColor),
