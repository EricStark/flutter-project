...
import 'package:first_flutter/page/web_page.dart';
...
import 'package:first_flutter/viewmodel/discovery/news_viewmodel.dart';
...
class _NewsPageState
  ...
  @override
  Widget getContentChild(NewsViewModel model) => ListView.builder(
  ...
  List<Widget> _getTitleList(List<String> titleList) {
    return titleList.map((title) {
      return Padding(
        ...
        child: Text(title,
            style: TextStyle(color: ConstColor.desTextColor, fontSize: 12),
            // 做多显示三行
            maxLines: 3,
            // 超出的部分，省略号处理
